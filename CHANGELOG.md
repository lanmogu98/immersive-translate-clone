...
-   **Duplicate Translation in Nested Custom Elements**: Fixed bug where `<h2>` containing `<body-text>` would cause both elements to be translated separately, resulting in duplicate translations. Changed `LEAF_CONTAINERS` to use lowercase tag names for cross-browser compatibility with custom element selectors.
